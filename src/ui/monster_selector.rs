...
pub fn draw_monster_selector(state: &GameState, x: f32, y: f32, w: f32, h: f32) -> Option<String> {
    ...
    for template in &templates {
        ...
        if unlocked {
            ...
            let label = format!(
                "{} {} ({}💎)",
                template.emoji, template.name, template.base_cost
            );
            ...
            draw_surface(Rect::new(x + 5.0, btn_y, w - 10.0, btn_h), &surface);
            draw_text(
